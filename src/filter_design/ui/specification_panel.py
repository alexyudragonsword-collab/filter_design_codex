...
class SpecificationPanel(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        group = QGroupBox("Design Specification")
        form = QFormLayout(group)
        form.setFieldGrowthPolicy(QFormLayout.FieldGrowthPolicy.AllNonFixedFieldsGrow)

        self.response = QComboBox()
        self.response.addItem("Low-pass", ResponseType.LOWPASS)
        self.response.addItem("High-pass", ResponseType.HIGHPASS)
        self.response.addItem("Band-pass", ResponseType.BANDPASS)
        self.response.addItem("Band-stop", ResponseType.BANDSTOP)
        self.approximation = QComboBox()
        self.approximation.addItem("Butterworth (maximally flat)", Approximation.BUTTERWORTH)
        self.approximation.addItem("Chebyshev I (equiripple)", Approximation.CHEBYSHEV1)
        ...
        self.order.setSpecialValueText("Automatic")
        self.order.setValue(0)

        form.addRow("Response Type", self.response)
        form.addRow("Approximation", self.approximation)
        form.addRow("Passband Edge 1", self.pass1)
        form.addRow("Passband Edge 2", self.pass2)
        form.addRow("Stopband Edge 1", self.stop1)
        form.addRow("Stopband Edge 2", self.stop2)
        form.addRow("Passband Ripple / Loss", self.ripple)
        form.addRow("Minimum Stopband Attenuation", self.attenuation)
        form.addRow("Port Impedance", self.impedance)
        form.addRow("Filter Order", self.order)
        layout.addWidget(group)

        self.design_button = QPushButton("Synthesize and Analyze")
        ...
        note = QLabel("Synthesis uses ideal lossless LC components. Verify component Q, parasitics, tolerances, power handling, and layout before manufacturing.")
    ...
    def set_busy(self, busy: bool) -> None:
        ...
        self.design_button.setText("Calculating…" if busy else "Synthesize and Analyze")
